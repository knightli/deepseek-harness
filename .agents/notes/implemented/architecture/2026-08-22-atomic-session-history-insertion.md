...
`@deepseek-ai/dsh-session` owns a provider-neutral atomic history capsule. `Session.insertHistoryGroup()` accepts a stable receipt, a stable logical anchor, and one or more complete closed turns. It appends exactly one required `session/history-insert` physical record. Ordinary `Session.append()` excludes and runtime-rejects that control type, so callers cannot publish partially assembled capsules.
...
**Add a general history-edit DSL.** Rejected for this seam because move/delete/replace operations are not required. A single anchored whole-group insertion is smaller, validates as one domain operation, and can be extended later only with a new demonstrated need.
...
- Insertions are intentionally limited to complete closed turns at a stable later `turn/start` anchor. Mid-turn editing, deletion, arbitrary reordering, and concurrent multi-writer merging remain unsupported and fail closed.
