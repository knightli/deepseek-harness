...
`apps/web/tests/external-text-agent.e2e.ts` pins the assembled application boundary. It boots the shipped base and Web layers, disables only the concrete `agent-loop` row, and mounts a deterministic external `AgentFactory` through a Loader `cordis:` row resolved against the same production package graph as the rest of the composition. Through the public Host HTTP wire it creates a session, reads its models, queues and steers text, cancels, renames, and reads history; it then tears down that complete Host, boots a second Host over the same workspace, persistence root, and harness home, forces `AgentFactory.resume`, and delivers a third prompt. The protocol golden records standard Inbox and user/assistant `SessionEvent` projections plus the create/resume lifecycle trace.

The same scenario preserves the negative control: a stock AgentLoop configured for the unserved external provider reports `routable: false`, returns the exact `model-unavailable` refusal, and appends no conversation history. Its native full-frame ARIA golden contains the shipped workspace/session sidebar, conversation messages, header/status surfaces, and enabled composer; no custom conversation surface or client store participates.
