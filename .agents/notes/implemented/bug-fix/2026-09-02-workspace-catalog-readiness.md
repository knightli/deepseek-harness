...
`WorkspaceBrowser` derives one catalog readiness value directly from the Session and Workspace snapshots. It withholds the tree and catalog actions until `baselinesReady` is true, preserves the last complete tree during later refreshes, and presents stable retry copy without raw transport errors. Retry invokes the two existing runtime refresh methods; Host implementations retain authority over request coalescing.

This is presentation coordination over existing object-layer facts. It creates no second catalog store and no independent readiness authority.
...
**Copy catalog data into a browser-owned store.** Rejected because it creates competing state, synchronization rules, and stale-data failure modes. The existing runtime snapshots already own every required fact.
