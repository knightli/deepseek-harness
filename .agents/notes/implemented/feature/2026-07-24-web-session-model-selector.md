...
The session RPC domain exposes a `session.models` directory, explicit `session.activate`, and `session.selectModel`. The directory is built dynamically from the LLM registry and grouped by provider; each listed model's exact metadata adds adapter-owned reasoning effort ids, names, descriptions, and optional default. Provider catalogs and exact metadata load concurrently by provider and fail independently, so successful groups remain usable alongside retryable failure records. Catalog membership stays advisory: `session.models.current` is returned independently and can remain routable when absent from every group, but the Host does not synthesize an unlisted row after its provider stops advertising it. An external authority that can publish the directory before it knows the exact next-step selection returns `current: null`; the catalog remains available and no row is active. An explicit activation can then establish interactive authority and publish the exact current selection without sending a prompt. The two surfaces answer a known-but-unlisted selection differently: the TUI renders it as its own row, while Web shows the unset trigger label and asks for a replacement. Web is the surface where a catalog is edited, so an absent directory row presents a selection decision; the TUI only picks from existing rows. A Web composer showing the unset label can still send when the Host separately reports the session as routable and no owner has blocked its input. Exact resolution decides whether a provider/model pair and explicit effort are available. Selection uses `resolveCallConfig` to reject unsupported effort ids and materialize an adapter-configured default before assigning the selection.

For a live Agent declaring `promptExecution: { kind: 'external-text' }`, the directory is compatibility display only. `session.models` reports ordinary text as routable without a DSH adapter, while `session.selectModel` returns `model-unavailable` and retains the current session selection and deployment default ([decision](../architecture/2026-08-14-external-text-prompt-execution.md)).

The browser `ModelDirectoryResolver` owns one `ModelDirectory` per live session. Its snapshot contains the current `ModelSelection | null`, grouped catalog, provider failures, operation error, and `idle`/`loading`/`ready`/`selecting`/`error` state. Mounting primes the trigger label and each menu open refreshes the directory. Directory and selection calls share an operation generation so an older response cannot replace a newer result; connection reset discards the process-local projection before restoring the Host selection. Failures retain the previous selection and usable groups.
