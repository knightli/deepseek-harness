...
Deployments can project one externally owned conversation into stock DSH Session/history/title views and resume it by exact binding without duplicating execution state. Tail reads and accepted renames become asynchronous authority boundaries and can fail closed. Providers must serialize refresh and rename for one Session and drain those operations during unload. The Host tests pin public error convergence and exact external-title projection; deployment-level runnable Host tests must additionally prove the concrete provider keeps one runtime owner while refreshing.
