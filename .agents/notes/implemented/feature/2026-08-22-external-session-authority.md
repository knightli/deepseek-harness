...
Catalog refresh and transcript refresh remain separate. `refreshCatalog()` may establish header-only Session rows, and `listMetadata(sessionId)` synchronously exposes a `nonBlank: true` hint from the last successful refresh so those externally confirmed conversations remain visible before their transcript is projected. The hint cannot mark a locally non-blank Session as blank, and the Host performs no external I/O while summarizing rows.
...
Deployments can project one externally owned conversation into stock DSH Session/history/title views and resume it by exact binding without duplicating execution state. A successful catalog refresh can expose the row immediately while its complete history remains lazy. Tail reads, writer acquisition, and accepted renames become asynchronous authority boundaries and can fail closed. Providers must serialize refresh and rename for one Session and drain those operations during unload. The Host tests pin catalog visibility, public error convergence, the `thread-busy` wire branch, and exact external-title projection; deployment-level runnable Host tests must additionally prove the concrete provider keeps one runtime owner while refreshing.
