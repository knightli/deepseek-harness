...
Per-preset agent composition. A **preset** is a directory holding one `agent.cordis.yml`; one live roster generation mounts it once under a standing scope, and each session that names it joins by having its agent scope key parented to the mount's (`dsh-scope`'s parent chain). The mount's tools, prompt sections, and projection units exist exactly once in that roster generation and cover every joined agent — its plugins key their state by Session/Agent, so sessions stay apart inside one shared instance — and a host reader with no agent at all (a cold transcript read) resolves the same standing registrations by preset id.
...
- `ctx.agentPresets.mount(agentCtx, id?): Promise<AgentPreset>` Low-level composition primitive: ensure a standing mount (single-flight), parent the agent's scope key to it, and return the preset. It does not protect a later Agent publication from roster teardown; an `AgentSetup` that will publish the agent MUST use `mountForPublication()` instead. Refuses a broken preset up front with its discovery-reported reason, so every unloadable shape fails the same way before the loader is involved.
- `ctx.agentPresets.mountForPublication(agentCtx, id?): Promise<AgentSetupCommit>` Compose an unpublished agent and return the receipt its `AgentSetup` must return. The factory commits the receipt synchronously after all setup awaits and immediately before publication; it revalidates the exact roster generation, standing mount, scope ancestry, and binding, so teardown rejects and rolls the whole creation back instead of publishing a host-composed agent.
...
### Where to call `mountForPublication()`

The agent factory's `setup(agentCtx)` hook is the one supported publication call site. It calls `mountForPublication()` and returns that receipt from setup. Only there is the join installed while the agent is still unpublished, and only the receipt's commit closes the setup-to-publication race: a rejected or revoked composition rolls the whole creation back rather than leaving a half-composed session. The standing subtree is owned by the roster service's own fiber — deliberately its UNTRACED context, because a subtree minted from a traced `this.ctx` resolves every service through the caller's shadow fiber instead of each entry's own inject store — so it survives every agent within that roster generation and unwinds when the roster unloads or restarts. Each standing generation records its composition file's stamp (mtime and size): a session that finds the stamp stale starts the next generation, while every session already joined keeps the one it runs on until that roster generation ends — the composition a running session joined outlives its file changing or disappearing underneath it, and files are the only composition editor, so the stamp is what carries an edit to later sessions.
...
Prefix-stable while the active roster generation owns the agent's standing composition: the composition is installed once, before the agent is published and therefore before its first request, and is not re-read during that generation. Choosing a different preset for a new session establishes a different prefix for that session alone. Roster unload or restart tears down existing standing compositions, so this package does not promise cache-prefix stability for those agents after that generation ends.
...
- **A file-superseded generation is not reclaimed while its roster generation lives** — sessions already joined keep the file generation they run on, and the roster holds no join count that could tell when the last one left. The cost is per file generation rather than per session: `dsh-skill-filesystem` watches its roots by default, so each edit-then-create cycle adds a live watcher set. Roster unload/restart tears every standing generation down, but does not migrate already-live agents to the replacement roster; those agents lose the roster-owned composition. Publication receipts prevent a new Agent from crossing that teardown boundary, not preserve existing agents through HMR. Reclaiming or migrating needs explicit joined-agent ownership outside the current roster fiber; see the `TODO` at `ensureStanding`.
