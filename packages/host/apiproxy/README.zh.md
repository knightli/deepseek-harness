...
会话模型选择属于会话领域约定。`session.models` 将当前 `ModelSelection` 与按提供方分组的建议性模型、精确模型的推理元数据和逐提供方查询失败记录分开返回。该选择可能不在这些分组中，也绝不会作为合成行注入；客户端可以提示用户作出另一项选择，而无需把目录变成路由白名单。`session.selectModel` 校验由适配器持有的可选推理强度，并指定下次组装提示词时使用的完整选择。目录成员关系不构成校验：适配器可以解析未列出的模型，而不可用的提供方或不受支持的推理强度会返回 `model-unavailable`。对于声明 `promptExecution: { kind: 'external-text' }` 的实时 Agent，目录仅用于客户端兼容显示：`session.selectModel` 会在适配器解析前返回 `model-unavailable`，且不会改变会话选择或部署默认值。`session.models.routable` 表示 Host 能否接收这个实时 Agent 的普通文本：所选提供方当前由 DSH 适配器服务，或该 Agent 声明了 `external-text`。该值不能从目录分组推导，也不承诺接收图片。`session.prompt` 会在开启轮次前执行相同的文本准入检查；不可用的注册表驱动路由返回 `model-unavailable`，`external-text` Agent 则通过自身 driver 接收文本。该 Agent 的图片内容会在持久化附件之前返回 `attachment-error`，原因为 `MODEL_DOES_NOT_SUPPORT_IMAGES`。客户端禁用 composer 只是提示性设计，这个方法始终可被调用（[决策](../../../.agents/notes/implemented/architecture/2026-08-14-external-text-prompt-execution.md)）。
