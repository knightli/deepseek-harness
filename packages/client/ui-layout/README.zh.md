...
AppFrame 通过 slot renderer 的注入钩子通道消费 connection 包的可观察状态。当状态为 `reconnecting` 时，stock `ConnectionBanner` 保持在应用子树之外，三栏网格自身的 reset `<fieldset>` 同时设为 disabled 与 inert，在不替换任何子 slot 的前提下阻止原生控件、pointer 操作与焦点；返回 `connected` 后会移除 banner，并解锁同一个已挂载子树。跨包设计理由由[重连 generation Agent Note](../../../.agents/notes/implemented/bug-fix/2026-08-16-reconnect-generation-interaction-ownership.md)负责。
