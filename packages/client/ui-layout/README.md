...
AppFrame consumes the connection package's observable state through the slot renderer's injected-hook channel. While state is `reconnecting`, the stock `ConnectionBanner` remains outside the application subtree and the three-column grid's own reset `<fieldset>` is both disabled and inert, blocking native controls, pointer actions, and focus without replacing any child slot. Returning to `connected` removes the banner and unlocks the same mounted subtree. The [reconnect generation Agent Note](../../../.agents/notes/implemented/bug-fix/2026-08-16-reconnect-generation-interaction-ownership.md) owns the cross-package rationale.
