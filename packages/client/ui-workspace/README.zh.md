...
浏览器把 Session 和 Workspace 的列表基线视作同一个目录。在两次首次成功拉取均到达前，它只渲染一处加载或重试状态，并隐藏不完整的目录树及其搜索、视图和添加操作。后续刷新会保留最近一次完整目录树，同时显示紧凑的进行中或旧结果提示。重试会请求两个列表服务；部署可以在 Host 边界合并这些调用。
...
Session 行内的 fork 操作在源会话最后一个已完成轮次处 fork，在客户端递增继承的持久化标题后再打开子会话；尾部半角或全角括号编号会原样式递增，无编号标题追加 ` (1)`。源会话与子会话在 workspace 组内始终作为同级行展示，谱系只保留为 session 数据。已明确证明的 Host 准入拒绝不会改变当前选中项，并通过原生 Toast 发布本地化提示，不渲染 carrier 原始文本。Host 已创建子会话、但后续设置失败时，另一条固定提示会明确说明子会话已存在但设置未完成；已创建的子会话仍留在列表中，当前选中项仍不移动。transport 失败无法证明发布结果时，UI 使用中性提示，要求先刷新 Session 列表再决定是否重试，避免错误声称没有子会话。每次发布都会重新挂载 Toast，而无关的 Workspace 重渲染不会重置其四秒生命周期。
