...
`SessionSummary.pendingInteraction` 将阻塞 Session 的实时用户操作分类为 `approval`、`plan-review` 或 `question`。`SessionManager` 依据稳定的请求标识跟踪 requested/resolved mux frame，即使 `Session` 对象尚未实例化也不例外；实例化前的缓冲会保留每个仍有效的请求，替换回放产生的重复项，并移除已解决的请求，因此打开 Session 时，列表状态会有一个对应的 `PendingWait`。审批与问题并发时，第一个 pending 问题具有更高的呈现优先级，以匹配 composer 路由；只有满足 plan-review composer 二元呈现约束的请求才会保留独立的 `plan-review` 状态。断连期间，已经实例化的 Session 会保留可见 wait 与列表状态，但在本地拒绝响应；实例化前的可应答 frame 会被丢弃，因为其失效 RPC id 没有已经渲染的 owner。新 generation 的回放会在 readiness 前重新标记仍然有效的请求；readiness 会删除未回放的旧状态，即使常驻 Session 仍处于 cold 状态也一样，同时保留已回放 wait，并且只允许已就绪 generation 响应。完整归属规则见[重连 generation Agent Note](../../../.agents/notes/implemented/bug-fix/2026-08-16-reconnect-generation-interaction-ownership.md)。
...
`SessionManager` 独立于列表和 Session 实例到达情况，保留最近一次通过验证的 `session/title` 控制快照。seq 更高的事件会替换旧快照，标题时间戳计入列表新近程度；订阅基线会先丢弃 seq 超过其 `lastSeq` 的任何已保留标题，再接收可选的折叠标题。显式移除 Session 也会清除已保留标题。因此，面向客户端的 `SessionSummary.title` 只包含实际的持久化标题；`displayTitle` 始终存在，并依次采用持久标题、Host 的可选外部 `displayTitleFallback`、cwd basename 和 Session id。该提示只用于列表，绝不会成为 `title`。冷态持久化会话会保持这个展示回退，直到打开或恢复会话，促使主机折叠并投影由日志支撑的标题。`ISession.rename` 用 unary 响应中的 `{title, seq}` 直接结算 `title` 投影格，遵循同一 seq 高者胜规则——列表行和所有 `useProjection('title')` 读者在推送帧到达前即更新；推送帧随后重放同一 seq 时为无操作。
