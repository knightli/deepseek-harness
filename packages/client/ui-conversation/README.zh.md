...
InputBar 与 ChatView 从 runtime Session 的 `sessionCapabilities` 快照字段读取图片和 fork 准入。每个 ready connection generation 中，Session 最多读取一次 `session.models.capabilities`；generation start 会同步清空该字段，迟到或失败的读取都不能发布。加载中、传输失败、响应无效，以及 Host 明确拒绝的能力因此都会渲染为不可用；此时粘贴与拖放会保留草稿和附件栏，并提示按 locale 解析的 `session.imageInputUnavailable` 文案，transcript 则不显示分支操作。切换 Session 对象或 connection generation 后的首次 render 会直接不可用，不需要 UI 自建能力 store，也不依赖 passive effect 重置（[决策](../../../.agents/notes/implemented/bug-fix/2026-08-17-fail-closed-session-capability-admission.md)）。
