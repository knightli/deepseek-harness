...
InputBar 与 ChatView 从 `session.models.capabilities` 读取图片和 fork 准入。加载中、传输失败、响应无效，以及 Host 明确拒绝的能力都会渲染为不可用；此时粘贴与拖放会保留草稿和附件栏，并提示 `This session does not support image input.`，transcript 则不显示分支操作。已加载快照同时以确切 `sessionId` 和能力读取器 identity 为键，因此切换会话或连接后的首次 render 会直接返回不可用快照，不会先暴露上一会话的控制项，再等待 passive effect 追上（[决策](../../../.agents/notes/implemented/bug-fix/2026-08-17-fail-closed-session-capability-admission.md)）。
