...
- `session.insertHistoryGroup({ receipt, before, members })` atomically appends one required physical capsule for complete closed turns or steps placed before a matching stable logical `turn/start` or `step/start`; exact receipt retries are no-ops and conflicts fail closed. Ordered insertion is a preparation-time operation: finish it on a detached Session before `ctx.sessions.enter()` publishes that Session. An attached/live Session rejects insertion with `LIVE_SESSION_UNSUPPORTED`.
- `session.history` exposes immutable logical `entries` and contiguous presentation-order `events`; `materializeSessionHistory(events)` provides the same canonical fold for detached physical logs. Host pagination keeps every insertion receipt group indivisible, and Host fork succeeds only when an exact physical prefix rematerializes to the requested logical prefix. See the [ordered-history decision](../../../.agents/notes/implemented/architecture/2026-08-22-atomic-session-history-insertion.md).
