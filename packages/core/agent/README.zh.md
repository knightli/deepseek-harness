...
- 高级有序生命周期：`enter(agent, owner, publication?): () => void` 强制 `agent.id === agent.session.id`，执行权威 ID 冲突检查，并在不通知的情况下插入；`owner` 显式记录实时创建方 agent 关系（根 agent 为 `undefined`），与持久会话谱系无关。直接发布的工厂会提供当前注册返回的确切 `AgentFactoryRegistration`；陈旧、外来、经过包装或原始工厂对象形式的值都会在插入前被拒绝。省略 `publication` 时，只会捕获外围 `ctx.agents.create()` 或 `resume()` 的活跃调用，并在该 Promise 结算时终止，绝不会仅因活跃工厂具备能力而借用其声明。`announce(agent)` 恰好发出一次 `agent/created`。创建监听器同步请求的 detach 会延后到该次分发结束；每次 detach 都会检查捕获的条目对象，因此陈旧能力无法删除后续使用同一 ID 的替代项。工厂发布使用这一拆分；普通插件使用 `register()`。
...
Agent *创建* 由实现 `AgentFactory` 的插件（`dsh-agent-loop`）提供，并通过 `setFactory` 注册。这样，创建功能留在 `dsh-agent` 接口上，消费方（UI、ACP（Agent Client Protocol）桥接层）可以面向 `ctx.agents` 编程，而不依赖具体循环包。注册表会把已经 traced 的 Service 规范化为具体目标，为该注册快照其会话能力，并通过调用方上下文重新 trace 每次调用；这既避免嵌套 Cordis shadow，也会把显式、绑定调用方的 `ownerCtx` 传给普通工厂。

`AgentFactory.sessionCapabilities` 可选择声明工厂级会话操作。seeded fork 准入有两道门。第一道门由 `ctx.agents.sessionCapabilities(id)` 建立 source eligibility：实时 Agent 读取发布它的注册快照，冷会话读取将要恢复它的活跃注册快照，且都不会创建或恢复 Agent。第二道门位于执行边界：每次 `options.seed` 存在（包括空 seed）的 `ctx.agents.create()`，都会在调用工厂前要求该调用捕获的确切执行槽位声明 `forkFromSeed: true`；false 或省略会抛出 `AgentFactoryCapabilityUnavailableError`。因此，在注册 A 下获准的有能力 source 不能交给无能力的替代注册 B；如果 B 自身也明确声明 true，则可以执行。经注册表包装的创建与恢复只会在其返回 Promise 结算前携带捕获的注册；直接发布的工厂则向 `enter()` 提供不透明的注册凭据。直接注册的外部 Agent 会省略凭据，因此即使活跃工厂具备能力也仍视为未声明。图片输入和模型选择仍是由 Host 从实时 Agent 推导的事实，而不是工厂字段（[决策](../../../.agents/notes/implemented/bug-fix/2026-08-17-fail-closed-session-capability-admission.md)）。

- `ctx.agents.setFactory(factory: AgentFactory): AgentFactoryRegistration`：注册创建工厂（循环在构造时调用），并把确切的单次 effect disposer 作为不透明的直接发布凭据返回。第二个工厂会导致抛出；dispose 时清空槽位。基于 Service 的直接发布方应把这一可调用凭据保留在 closure、模块私有 `WeakMap` 或普通 holder 中，因为经 Cordis tracing 读取函数值 Service 字段会改变其身份。
- `ctx.agents.create(options: CreateAgentOptions): Promise<AgentHandle>`：捕获当前工厂槽位，任何 seeded 调用都必须先由该槽位精确声明 `forkFromSeed: true`，然后才会到达工厂；之后创建会话和 agent，在不发布的情况下等待可选 setup，并通过最终的 `SessionStore.enter()` 与 `AgentRegistry.enter()` 检查发布。不支持的 seeded 调用会在工厂执行前抛出 `AgentFactoryCapabilityUnavailableError`。不支持并发创建同一 ID：多个操作可以进行准备，但只有一个能进入；每个失败方都会回滚其私有作用域／会话／驱动器。可选且只用于创建的 `signal` 会取消未发布的 setup，并在返回 handle 前分离；之后的取消使用 `handle.dispose()` 或 `agent.cancel()`。发布包含在回滚范围内，回滚期间每条已交付创建边都会成对处理。环境中的注册凭据会在此 Promise 结算时失效，因此分离的 continuation 无法发布。未注册工厂时拒绝。
- `ctx.agents.resume(options: ResumeAgentOptions): Promise<AgentHandle>`：加载持久化会话（[会话持久化](../../../.agents/notes/implemented/architecture/2026-06-14-session-persistence.md)），创建新的未发布 agent 作用域，等待可选 setup，并使用相同的最终进入发布序列。其可选 `signal` 同样只用于创建，环境中的注册凭据也会在此 Promise 结算时失效。未注册工厂或未配置会话持久化时拒绝。
