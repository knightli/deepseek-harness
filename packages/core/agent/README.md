...
`AgentFactory.sessionCapabilities` optionally declares factory-wide session operations. Omission preserves the stock behavior, including seeded forks; `forkFromSeed: false` denies generic fork admission for both live and cold sessions. `ctx.agents.sessionCapabilities(id)` reads the capability from the factory that published a live Agent, or from the active factory that would resume a cold session, without creating or resuming an Agent. Image input and model selection remain live-Agent facts derived by the Host rather than factory fields ([decision](../../../.agents/notes/implemented/bug-fix/2026-08-17-fail-closed-session-capability-admission.md)).
