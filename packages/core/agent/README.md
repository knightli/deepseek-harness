...
- Advanced ordered lifecycle: `enter(agent, owner, publisher?): () => void` enforces `agent.id === agent.session.id`, performs the authoritative ID collision check, and inserts without announcing; `owner` explicitly records the live creator-agent relation (or `undefined` for a root), independently of durable session lineage. A direct factory publisher must be the exact canonical factory in the current registry slot; it records only that factory's declared session capabilities, while omission captures only a surrounding `ctx.agents.create()` or `resume()` call and never borrows capabilities merely from the active factory. `announce(agent)` emits `agent/created` exactly once. A detach requested synchronously by a creation listener is deferred until that dispatch unwinds, and every detach checks the captured entry object, so a stale capability cannot delete a later same-ID replacement. Factory publication uses this split; ordinary plugins use `register()`.
...
`AgentFactory.sessionCapabilities` optionally declares factory-wide session operations. Seeded fork admission requires `forkFromSeed: true`; false or omission denies generic fork admission for both live and cold sessions. `ctx.agents.sessionCapabilities(id)` reads the declaration captured from the factory that published a live Agent, or from the active factory that would resume a cold session, without creating or resuming an Agent. Registry-wrapped create and resume capture it automatically; direct factory publication identifies the exact registered factory through `enter()`. Direct external registration omits that identity and remains undeclared even while a capable factory is active. Image input and model selection remain live-Agent facts derived by the Host rather than factory fields ([decision](../../../.agents/notes/implemented/bug-fix/2026-08-17-fail-closed-session-capability-admission.md)).
