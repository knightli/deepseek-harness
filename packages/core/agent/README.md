...
- Advanced ordered lifecycle: `enter(agent, owner, publication?): () => void` enforces `agent.id === agent.session.id`, performs the authoritative ID collision check, and inserts without announcing; `owner` explicitly records the live creator-agent relation (or `undefined` for a root), independently of durable session lineage. A direct factory publisher supplies the exact `AgentFactoryRegistration` returned for its current registration; a stale, foreign, wrapped, or raw-factory value is rejected before insertion. Omission captures only the active invocation of a surrounding `ctx.agents.create()` or `resume()`, ending when that Promise settles, and never borrows capabilities merely from the active factory. `announce(agent)` emits `agent/created` exactly once. A detach requested synchronously by a creation listener is deferred until that dispatch unwinds, and every detach checks the captured entry object, so a stale capability cannot delete a later same-ID replacement. Factory publication uses this split; ordinary plugins use `register()`.
...
Agent *creation* is provided by the plugin implementing `AgentFactory` (`dsh-agent-loop`), registered via `setFactory`. This keeps creation on the `dsh-agent` interface so consumers (UI, the ACP bridge) program against `ctx.agents` without depending on the concrete loop package. The registry canonicalizes an already traced Service to its concrete target, snapshots its session capabilities for that registration, and re-traces each call through the caller's context; this avoids nested Cordis shadows while passing an explicit caller-bound `ownerCtx` to plain factories.

`AgentFactory.sessionCapabilities` optionally declares factory-wide session operations. Seeded fork admission has two gates. First, `ctx.agents.sessionCapabilities(id)` establishes source eligibility from the registration snapshot that published a live Agent, or from the active registration snapshot that would resume a cold session, without creating or resuming an Agent. Second, every `ctx.agents.create()` whose `options.seed` is present (including an empty seed) requires `forkFromSeed: true` on the exact execution slot captured by that call before invoking its factory; false or omission throws `AgentFactoryCapabilityUnavailableError`. A capable source admitted under registration A therefore cannot be handed to an incapable replacement B, while a replacement that independently declares true may execute it. Registry-wrapped create and resume carry their captured registration only until the returned Promise settles; direct factory publication supplies its opaque registration receipt to `enter()`. Direct external registration omits a receipt and remains undeclared even while a capable factory is active. Image input and model selection remain live-Agent facts derived by the Host rather than factory fields ([decision](../../../.agents/notes/implemented/bug-fix/2026-08-17-fail-closed-session-capability-admission.md)).

- `ctx.agents.setFactory(factory: AgentFactory): AgentFactoryRegistration` — register the creation factory (the loop calls this on construction) and return the exact single-shot effect disposer as its opaque direct-publication receipt. Throws on a second factory; the slot clears on dispose. A Service-backed direct publisher retains the callable receipt in a closure, module-private `WeakMap`, or plain holder because reading a function-valued Service field through Cordis tracing changes its identity.
- `ctx.agents.create(options: CreateAgentOptions): Promise<AgentHandle>` — capture the current factory slot, require its exact `forkFromSeed: true` declaration before any seeded call reaches the factory, create a session and agent, await optional setup while unpublished, then publish through final `SessionStore.enter()` and `AgentRegistry.enter()` checks. An unsupported seeded call throws `AgentFactoryCapabilityUnavailableError` before factory invocation. Concurrent same-ID creation is unsupported: more than one operation may prepare, but only one can enter; every loser rolls its private scope/session/driver back. An optional creation-only `signal` cancels unpublished setup and is detached before the handle is returned; later cancellation uses `handle.dispose()` or `agent.cancel()`. Publication is rollback-covered and every delivered creation edge is paired during rollback. The ambient registration receipt expires when this Promise settles, so detached continuations cannot publish. Rejects if no factory is registered.
- `ctx.agents.resume(options: ResumeAgentOptions): Promise<AgentHandle>` — load a persisted session ([session persistence](../../../.agents/notes/implemented/architecture/2026-06-14-session-persistence.md)), mint a fresh unpublished agent scope, await optional setup, and use the same final-entry publication sequence. Its optional `signal` is likewise creation-only, and its ambient registration receipt expires when this Promise settles. Rejects if no factory is registered or session persistence is unconfigured.
