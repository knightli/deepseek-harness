...
Its factory declaration sets `sessionCapabilities.forkFromSeed: true`: the `createAgent()` path validates and snapshots a supplied seed, so the Host may truthfully admit ordinary Session forks without relying on an implicit default. AgentLoop captures the exact callable receipt returned by `setFactory()` at each create, resume, or restore-or-create operation entry and retains it across setup and persistence awaits. Programmatic publication must still match the registry's ambient invocation, while declarative `sessionId` creation and `resumeSessionId` restoration supply that captured receipt directly; both routes therefore retain the same registration snapshot on the live entry and reject a late publication after replacement.
