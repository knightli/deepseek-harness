...
Its factory declaration sets `sessionCapabilities.forkFromSeed: true`: the `createAgent()` path validates and snapshots a supplied seed, so the Host may truthfully admit ordinary Session forks without relying on an implicit default. Programmatic create and resume publish inside the registry's factory call, while declarative `sessionId` create and `resumeSessionId` restore identify this exact registered factory when they publish directly; both routes therefore retain the same declaration on the live entry.
