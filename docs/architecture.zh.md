...
| [`core/agent`](subsystems/core.md) | `Agent` 接口、活跃 agent 注册表、工厂注册 epoch、seeded-create 能力门和 `agent/*` 事件 | `ctx.agents` |
| [`core/agent-loop`](subsystems/core.md) | 实现该接口的默认驱动器与精确注册直接发布方 | `ctx.agentLoop` |
