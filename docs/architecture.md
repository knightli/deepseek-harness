...
| [`core/agent`](subsystems/core.md) | The `Agent` interface, live registry, factory-registration epochs, seeded-create capability gate, and `agent/*` events | `ctx.agents` |
| [`core/agent-loop`](subsystems/core.md) | The default driver and exact-registration direct publisher implementing that interface | `ctx.agentLoop` |
